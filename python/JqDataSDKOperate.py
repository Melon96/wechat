...
def set_sw1_pe_and_pb_to_mysql(code, Count):
    conn = pymysql.connect('123.207.26.168', 'root', 'WANGfei123')
    conn.select_db('sw1')
    cur = conn.cursor()  # 获取游标
    str = "insert into sw1_" + code
    df = jqdatasdk.finance.run_query(
        query(jqdatasdk.finance.SW1_DAILY_VALUATION).filter(
            jqdatasdk.finance.SW1_DAILY_VALUATION.code == code).order_by(
            jqdatasdk.finance.SW1_DAILY_VALUATION.date.desc()).limit(Count))
    ...
    framePe = pd.DataFrame(df['pe'])
    framePb = pd.DataFrame(df['pb'])
    frameDate = pd.DataFrame(df['date'])
    frameName = pd.DataFrame(df['name'])
    TodayName = frameName[0:1]
    strGetName = '%s ' % TodayName
    nEnd = len(strGetName)
    strGetName = strGetName[nEnd - 7:nEnd]
    strName = strGetName.strip()
    nCount = 0

    for (pe, pb, date) in zip(framePe.iterrows(), framePb.iterrows(), frameDate.iterrows()):
        sql = str + " (date,name,pe,pb) values(%s,%s,%s,%s)"
        Date = date[1]
        strDate = '%s ' % Date
        strDate = strDate[8:18]
        fpe = float(pe[1])
        strpe = "{:.2f}".format(fpe)
        fpb = float(pb[1])
        strpb = "{:.2f}".format(fpb)
        cur.execute(sql, (strDate, strName, strpe, strpb))
        nCount += 1
    print(nCount)
    cur.close()
    conn.commit()
...
# set_sw1_pe_and_pb_to_mysql("801740", 1)
get_sw1_pe_and_pb("801740", 1)
